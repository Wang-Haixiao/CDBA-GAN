...
def generator_SN( latent_dim, image_shape, num_classes, num_res_blocks, base_name):
    ...
    # # se = SeNetBlock()(x1)
    # # x = add([cam, se])
    # x2 = SelfAttention(ch=128 * 2, name=base_name + "_sa")(x)
    ...
    # se = SeNetBlock()(x1)
    # x = add([cam, se])
    x2 = SelfAttention(ch=128, name=base_name + "_sa")(x)
...
# #
    ...
    # se = SeNetBlock()(x1)
    # x = add([cam, se])
    x2 = SelfAttention(ch=128, name=base_name + "_sa")(D)
    x = Conv2D(128, 3, padding='same', use_bias=False)(x2)
    # x = BatchNormalization(axis=3)(x)
    x = Activation("relu")(x)
    x = Dropout(0.5)(x)
