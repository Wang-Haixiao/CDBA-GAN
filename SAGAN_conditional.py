...
import cv2
import utils
# import dual_att_net_utils_gen256
# import dual_att_net_utils_gen64
import multi_att_net_utils_two
# import multi_att_net_utils_dense
import dual_att_net_utils_gen64
#import dual_att_net_utils_gen16_16
# import dual_att_net_utils_gen128_128
# import CAM_att_net_utils
# import SE_net_utils
# import net_utils
...
class SAGAN():
    ...
    def __init__(self, config):
        self.config = config
        self.img_rows = 256  # 512
        self.img_cols = 192  # 384
    ...
    def build_model(self, discriminator_path=None, generator_path=None):
        ...
        if discriminator_path:
            ...
            self.discriminator = dual_att_net_utils_gen64.discriminator_SN(self.config.num_classes,self.config.IMAGE_SHAPE, base_name="discriminator",
                                                     use_res=self.config.USE_RES )#multi_att_net_utils
        if generator_path:
            self.generator = load_model(generator_path)
        else:
            self.generator = dual_att_net_utils_gen64.generator_SN(self.config.LATENT_DIM, self.config.IMAGE_SHAPE, self.config.num_classes,
    ...
    def load_data(self):
        ...
        columnar_v1 = int(155 * percent)#155
        linear_v1 = int(64 * percent)#64
        ...
        train_labels = np.array(
            [0] * anger + [1] * disgust + [2] * columnar_v1 + [3] * linear_v1 )
        test_labels = np.array(
            [0] * (128 - anger) + [1] * (64 - disgust) + [2] * (155 - columnar_v1) + [3] * (64 - linear_v1))
        train_data = np.zeros((328, self.img_rows, self.img_cols, self.channels), dtype=np.uint8)#328
        test_data = np.zeros((83, self.img_rows, self.img_cols, self.channels), dtype=np.uint8)#83
        # train_labels = np.array(
        ...
        # imgs = os.listdir(self.root + "/dual_1_124/")#columnar_mul_improv
        # num = len(imgs)
        # for i in range(num):
        #     img = cv2.imread(self.root + '/dual_1_124/%s' % imgs[i], 0)
        #     #img = cv2.resize(img, (self.img_rows, self.img_cols))
        #     img = cv2.resize(img, (self.img_cols, self.img_rows))
        #     # plt.imshow(img)
        #     arr = np.asarray(img, dtype='float32')
        ...
        #         index1 += 1
        #
        # imgs = os.listdir(self.root + "/dual_2_156/")#linear_mul
        # num = len(imgs)
        # for i in range(num):
        #     img = cv2.imread(self.root + '/dual_2_156/%s' % imgs[i], 0)
        #     #img = cv2.resize(img, (self.img_rows, self.img_cols))
        #     img = cv2.resize(img, (self.img_cols, self.img_rows))
        #     arr = np.asarray(img, dtype='float32')
        #     arr = arr.reshape(self.img_rows, self.img_cols, self.channels)
    ...
    def train_iterations(self, counter=0):
        ...
        for epoch in range(self.config.EPOCH):
            ...
            for iter in range(self.config.ITER_PER_EPOCH):
                ...
                noise = np.random.normal(size=(self.config.BATCH_SIZE, self.config.LATENT_DIM))
                sampled_labels = np.random.randint(0, 4, (self.config.BATCH_SIZE, 1))
                ...
                if counter % 500 == 0:
                    ...
                    # sample の出力
                    sample = self.generator.predict([fixed_noise, sampled_labels])  #stupid
        ...
        met_curve.to_csv(os.path.join(experiment_dir,
                         ...
                         indemulti_att_net_utils_newx=False)



    def generate(self,label, weights_path, number_images):
        generator = dual_att_net_utils_gen16_16.generator_SN(self.config.LATENT_DIM, self.config.IMAGE_SHAPE,self.config.num_classes,
                                             self.config.NUMBER_RESIDUAL_BLOCKS, base_name="generator")
