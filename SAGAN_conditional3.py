...
import cv2
import utils
# import dual_att_net_utils
import multi_att_net_utils
# import net_utils
...
class SAGAN():
    ...
    def build_model(self, discriminator_path=None, generator_path=None):
        ...
        if discriminator_path:
            ...
            self.discriminator = multi_att_net_utils.discriminator_SN(self.config.num_classes,self.config.IMAGE_SHAPE, base_name="discriminator",
                                                     use_res=self.config.USE_RES )
        if generator_path:
            self.generator = load_model(generator_path)
        else:
            self.generator = multi_att_net_utils.generator_SN(self.config.LATENT_DIM, self.config.IMAGE_SHAPE, self.config.num_classes,
    ...
    def load_data(self):
        ...
        columnar_v1 = int(124 * percent)#155
        linear_v1 = int(156 * percent)#64
        ...
        train_labels = np.array(
            [0] * anger + [1] * disgust + [2] * columnar_v1 + [3] * linear_v1 )
        test_labels = np.array(
            [0] * (128 - anger) + [1] * (64 - disgust) + [2] * (124 - columnar_v1) + [3] * (156 - linear_v1))
        train_data = np.zeros((376, self.img_rows, self.img_cols, self.channels), dtype=np.uint8)#328
        test_data = np.zeros((96, self.img_rows, self.img_cols, self.channels), dtype=np.uint8)#83
        # train_labels = np.array(
    ...
    def generate(self,label, weights_path, number_images):
        generator = multi_att_net_utils.generator_SN(self.config.LATENT_DIM, self.config.IMAGE_SHAPE,self.config.num_classes,
                                             self.config.NUMBER_RESIDUAL_BLOCKS, base_name="generator")
