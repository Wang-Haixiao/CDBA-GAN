...
from keras.layers import Input, Dense, Activation, Flatten, Add, Lambda, Concatenate, Reshape, UpSampling2D, add
...
def generator_SN( latent_dim, image_shape, num_classes, num_res_blocks, base_name):
    ...
    x = Dense(64*8*h//16*w//16, activation="relu", name=base_name+"_dense")(model_input)
    x = Reshape((h//16, w//16, -1))(x)

    x = UpSampling2D((2, 2))(x)
    x = ConvSN2D(64*4, kernel_size=3, strides=1, padding='same', kernel_initializer=initializer,
                        name=base_name + "_conv1")(x)
    # x = Conv2DTranspose(64 * 4, kernel_size=3, strides=2, padding='same', kernel_initializer=initializer,
    #              name=base_name + "_conv1")(x)
    x = BatchNormalization(momentum=0.9, epsilon=1e-5, name=base_name + "_bn1")(x, training=1)
    x = Activation("relu")(x)
    cam = CAM()(x)
    x = Conv2D(256, 3, padding='same', use_bias=False)(cam)
    x = BatchNormalization(axis=3)(x)
    x = Activation("relu")(x)
    x = Dropout(0.5)(x)
    cam = Conv2D(256, 3, padding='same', use_bias=False)(x)

    # se = SeNetBlock()(x1)
    # x = add([cam, se])
    x2 = SelfAttention(ch=128 * 2, name=base_name + "_sa1")(x)
    x = Conv2D(256, 3, padding='same', use_bias=False)(x2)
    x = BatchNormalization(axis=3)(x)
    x = Activation("relu")(x)
    x = Dropout(0.5)(x)
    sa = Conv2D(256, 3, padding='same', use_bias=False)(x)
    ...
    # size//8→size//4→size//2→size
    x = UpSampling2D((2, 2))(x_1)
    x = ConvSN2D(64*2, kernel_size=3, strides=1, padding='same', kernel_initializer=initializer,
                        name=base_name + "_conv2")(x)
    # x = Conv2DTranspose(64 * 2, kernel_size=3, strides=2, padding='same', kernel_initializer=initializer,
    #              name=base_name + "_conv2")(x)
    x = BatchNormalization(momentum=0.9, epsilon=1e-5, name=base_name + "_bn2")(x, training=1)
    # x = Activation("relu")(x)
    x1 = Activation("relu")(x)
    cam = CAM()(x1)
    x = Conv2D(128, 3, padding='same', use_bias=False)(cam)
    x = BatchNormalization(axis=3)(x)
    x = Activation("relu")(x)
    x = Dropout(0.5)(x)
    cam = Conv2D(128, 3, padding='same', use_bias=False)(x)

    # se = SeNetBlock()(x1)
    # x = add([cam, se])
    x2 = SelfAttention(ch=64*2, name=base_name+"_sa2")(x)
    x = Conv2D(128, 3, padding='same', use_bias=False)(x2)
    x = BatchNormalization(axis=3)(x)
    x = Activation("relu")(x)
    x = Dropout(0.5)(x)
    sa = Conv2D(128, 3, padding='same', use_bias=False)(x)
    ...
    x_2 = x_2 + x_1
    # x = SelfAttention(ch=64*2, name=base_name+"_sa")(x1)

    x = UpSampling2D((2, 2))(x_2)
    x = ConvSN2D(64*1, kernel_size=3, strides=1, padding='same', kernel_initializer=initializer,
                        name=base_name + "_conv3")(x)
    # x = Conv2DTranspose(64 * 1, kernel_size=3, strides=1, padding='same', kernel_initializer=initializer,
    #              name=base_name + "_conv3")(x)
    x = BatchNormalization(momentum=0.9, epsilon=1e-5, name=base_name + "_bn3")(x,training=1)
    x1 = Activation("relu")(x)
    cam = CAM()(x1)
    x = Conv2D(64, 3, padding='same', use_bias=False)(cam)
    x = BatchNormalization(axis=3)(x)
    x = Activation("relu")(x)
    x = Dropout(0.5)(x)
    cam = Conv2D(64, 3, padding='same', use_bias=False)(x)

    # se = SeNetBlock()(x1)
    # x = add([cam, se])
    x2 = SelfAttention(ch=64, name=base_name + "_sa3")(x)
    x = Conv2D(64, 3, padding='same', use_bias=False)(x2)
    x = BatchNormalization(axis=3)(x)
    x = Activation("relu")(x)
    x = Dropout(0.5)(x)
    sa = Conv2D(64, 3, padding='same', use_bias=False)(x)
    ...
    x = UpSampling2D((2, 2))(x_3)
    out = ConvSN2D(1, kernel_size=3, strides=1, padding='same', activation="tanh",
                          kernel_initializer=initializer, name=base_name + "_out")(x)
    # out = Conv2DTranspose(1, kernel_size=3, strides=2, padding='same', activation="tanh",
    #                       kernel_initializer=initializer, name=base_name + "_out")(x)
...
def discriminator_SN(num_classes, input_shape, base_name,  use_res=False):
    ...
    # de1 = Reshape((512, 384, 1))(model_input)
...
##
    ...
    # se = SeNetBlock()(x1)
    # x = add([cam, se])
    x2 = SelfAttention(ch=64, name=base_name + "_sa1")(D)
    x = Conv2D(64, 3, padding='same', use_bias=False)(x2)
    # x = BatchNormalization(axis=3)(x)
    x = Activation("relu")(x)
    x = Dropout(0.5)(x)
    ...
    x = Conv2D(64, 3, padding='same', use_bias=False)(x)
    # x = BatchNormalization(axis=3)(x)
    D_1 = Activation("relu")(x)
# #
    # cam = CAM()(D)
    # D = SeNetBlock()(D)
    # x = add([cam, se])
    # x2 = SelfAttention(ch=64, name=base_name+"_sa")(D)
    # D = add([cam, x2])
    #
    #
    # D = SelfAttention(ch=64, name=base_name+"_sa")(D)
    ...
    # se = SeNetBlock()(x1)
    # x = add([cam, se])
    x2 = SelfAttention(ch=128, name=base_name + "_sa2")(D)
    x = Conv2D(128, 3, padding='same', use_bias=False)(x2)
    # x = BatchNormalization(axis=3)(x)
    x = Activation("relu")(x)
    x = Dropout(0.5)(x)
    ...
    # se = SeNetBlock()(x1)
    # x = add([cam, se])
    x2 = SelfAttention(ch=256, name=base_name + "_sa3")(D)
    x = Conv2D(256, 3, padding='same', use_bias=False)(x2)
    # x = BatchNormalization(axis=3)(x)
    x = Activation("relu")(x)
    x = Dropout(0.5)(x)
    ...
    # out = DenseSN(units=1, activation=sigmoid, name=base_name + "_out")(D)
