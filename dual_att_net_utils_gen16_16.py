...
def generator_SN( latent_dim, image_shape, num_classes, num_res_blocks, base_name):
    ...
    cam = CAM()(x)
    x = Conv2D(512, 3, padding='same', use_bias=False)(cam)
    x = BatchNormalization(axis=3)(x)
    x = Activation("relu")(x)
    x = Dropout(0.5)(x)
    cam = Conv2D(512, 3, padding='same', use_bias=False)(x)

    # se = SeNetBlock()(x1)
    # x = add([cam, se])
    x2 = SelfAttention(ch=256 * 2, name=base_name + "_sa")(x)
    x = Conv2D(512, 3, padding='same', use_bias=False)(x2)
    x = BatchNormalization(axis=3)(x)
    x = Activation("relu")(x)
    x = Dropout(0.5)(x)
    sa = Conv2D(512, 3, padding='same', use_bias=False)(x)
...
# #
    ...
    # se = SeNetBlock()(x1)
    # x = add([cam, se])
    x2 = SelfAttention(ch=512, name=base_name + "_sa")(D)
    x = Conv2D(512, 3, padding='same', use_bias=False)(x2)
    # x = BatchNormalization(axis=3)(x)
    x = Activation("relu")(x)
    x = Dropout(0.5)(x)
